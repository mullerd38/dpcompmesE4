...
stimulus: function() {
  ...
  if (condition === "two-step") {
    ...
    if (response == "more likely to recover after receiving the medicine") {
      questionText = "<p class='instructions'>You just said patients are more likely to recover after receiving the medicine than the placebo.<br>" +
      ...
      questionText = "<p class='instructions'>You just said patients are more likely to recover after receiving the placebo than the medicine.<br>" +
...
var conditional_slider = {
  ...
  conditional_function: function() {
    var response = jsPsych.data.get().last().values()[0].response.Q0;
    if (response == "equally likely to recover after receiving the medicine or the placebo") {
      return false;
    } else {
      return true;
    }
...
const experiment_id = "7h8aJMIZRkzZ";
// Your OSF token
